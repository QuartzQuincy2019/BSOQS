    ...
    */
    {
        ...
        contents: [
            ...
            "[c[\\|]c用于分隔单元格（此时它代表c[&lt;/td&gt;&lt;td&gt]c;或c[&lt;/th&gt;&lt;th&gt;]c），每行第一个单元格开头必须是c[|]c（此时它代表c[&lt;td&gt;]c或c[&lt;th&gt;]c），每行最后一个单元格结尾必须带1个c[|]c。]",
            "[一个标准的表格行如下：]",
            ...
            "[每个决定一个单元格的c[|]c后面跟c[\\{cMrN\\}]c（c[c]c, c[r]c不区分大小写且可以调换顺序），表示该单元格存在合并属性，不跟则没有。如：]",
            "[@example: c[|{c2} 表示 &lt;td colspan='2'&gt;]c； c[|{c2r2}]c表示c[&lt;td colspan='2' rowspan='2'&gt;]c。]",
            "[特别地，c[||]c等价于c[|$^|]c，代表c[&lt;/td&gt;&lt;/tr&gt;&lt;tr&gt;&lt;/td&gt;]c（或c[&lt;/th&gt;&lt;/tr&gt;&lt;tr&gt;&lt;/td&gt;]c）。]",
            ...
            "[c[\\%\\{\\}]c和c[\\ć\\{\\}]c表示类名，即c[class]c属性。其他的参数，如c[\\ŧ\\{\\}]c，表示c[title]c属性。]",
            "[不跟单字符的c[\\{\\}]c表示固有参数，每个标签只有一个指定的固有参数。比如c[A\\[\\]A]c表示c[&lt;a&gt;&lt;/a&gt;]c，它的固有参数是c[href]c，也可以用c[\\ĥ\\{\\}]c表示。c[v\\[\\]v]c的固有参数是c[class]c。]",
            "[@example: c[v\\[\\{red alert\\}alert\\]v]c 返回 c[&lt;span class='red alert'&gt;alert&lt;/span&gt;]c",
            "[@example: c[t\\[%\\{t1\\}]c 返回 c[&lt;table class='t1'&gt;]c]",
            "[对于单元格，c[\\|]c后的c[\\{\\}]c表示合并属性。这两个c[\\{\\}]c没有先后之分，仅靠c[\\%]c或c[\\ć]c识别。]",
