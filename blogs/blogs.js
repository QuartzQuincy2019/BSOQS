    ...
    */
    {
        ...
        contents: [
            ...
            "[%{TI}此时有人会问“v[{emphasis}为什么我不能全年看到北河二，但是全年都能看到北斗七星呢？]v”这就需要引入s[拱极星]s的概念。不过首先要明白一个前提：s[由于恒星距离地球极其遥远，所以视所有恒星的位置不变，且恒星散发的光为v[{emphasis}平行光]v]s。因此，把所有恒星散发的光全部s[投影]s到地球表面，在地球表面建立一个s[球面坐标系]s，便可以描述所有恒星的一切位置。一般地，我们用s[赤道坐标系]s描述恒星在天球的位置。]",
            ...
            "[%{TI}进一步地，在某一纬度的地点，s[拱极圈]s（恒显圈）内的星星永远可见。如果当地纬度为c[φ]c（不论南纬或北纬），拱极圈内的恒星的赤纬一定在c[\\[90°-φ,90°\\]]c内。由拱极圈的定义可以推知，s[拱极圈一定与当地地平线相切]s，且s[当地纬度越高，拱极圈范围越大]s。]",
            ...
            "[%{TI}比如，c[UMa]c是大熊座c[s[U]srsae s[Ma]sjoris]c的缩写，其中c[Ursae]c（熊）和c[Majoris]c（大）均为s[单数属格]s变格。c[CMi]c是小犬座c[s[C]sanis s[Mi]snoris]c的缩写，其中c[Canis]c（狗）和c[Minoris]c（小）均为s[单数属格]s变格。c[Gen]c是双子座c[s[Gem]sini]c的缩写，其中c[Gemini]c（双子）为s[单数属格]s变格。]",
            "h1[三、其他]h1",
