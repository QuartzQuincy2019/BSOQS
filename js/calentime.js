...
class CalenTime {
    ...
    moveDate(delta) {
        ...
        g = g.next(delta);
        st = SolarTime.fromYmdHms(g.year, g.month, g.day, st.hour, st.minute, st.second);
        this.dataArr = [st.year, st.month, st.day, st.hour, st.minute, st.second];
        return this;
    }
    moveWeek(delta) {
        let st = SolarTime.fromYmdHms(...this.dataArr);
    ...
    moveMonth(delta) {
        ...
        g = g.next(delta);
        st = SolarTime.fromYmdHms(g.year, g.month, st.day, st.hour, st.minute, st.second);
        this.dataArr = [st.year, st.month, st.day, st.hour, st.minute, st.second];
        return this;
    }
    moveYear(delta) {
    ...
    localize() {
        let date = new Date();
        this.dataArr = [date.getFullYear(), date.getMonth() + 1, date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds()];
        return this;
    }
}
...
class CalenOperator {
    ...
    getYearZodiac(calenTime) {
        return this.getSixtyCycleDay(calenTime).getYear().getEarthBranch().getZodiac();
    }
...
function fillCalendar() {
    ...
    if (operator.getSolarTermDay(moment).dayIndex == 0) {
        E_TermDay.innerText = operator.getSolarTermDay(moment).toString().slice(0, 2);
    } else {
        E_TermDay.innerText = operator.getSolarTermDay(moment).toString();
    }
    E_ThreePillars.innerText = operator.getThreePillars(moment).getYear() + '(' + operator.getYearZodiac(moment).toString() + ')年 ' + operator.getThreePillars(moment).getMonth() + '月 ' + operator.getThreePillars(moment).getDay() + '日';
    E_Land.innerText = operator.getDirection(moment).toString() + operator.getDirection(moment).getLand().toString();
    let twentyEightStar = operator.getTwentyEightStar(moment);
    E_28Star.innerText = operator.getZone(moment).toString() + operator.getBeast(moment).toString() + " " + twentyEightStar.toString() + twentyEightStar.getSevenStar().toString() + twentyEightStar.getAnimal().toString();
    E_Phase.innerText = operator.getPhase(moment).toString();
    E_Constellation.innerText = operator.getConstellation(moment).toString();
    E_YearIndex.innerText = '第' + (operator.getSolarDay(moment).getIndexInYear() + 1) + '日';
    E_TwelveStar.innerText = operator.getEcliptic(moment).toString() + " " + operator.getTwelveStar(moment).toString();
    ...
    E_Recommends.innerText = operator.getHourRecommends(moment).toSpliced(6).toLocaleString();
    E_Taboos.innerText = operator.getHourAvoids(moment).toSpliced(6).toLocaleString();
}
...
window.addEventListener("keydown", (e) => {
    ...
    if (keySet.includes(e.key)) {
        ...
        if (e.key != "0" && calendarRefresher) {// 如果按下的不是0且日历正在刷新，则按键无效
            return;
        }
        switch (e.key) {
            case "-": moment.moveTwoHours(-1);
                break;
            case "=": moment.moveTwoHours(1);
                break;
            case "o": moment.moveDate(-1);
                break;
            case "p": moment.moveDate(1);
                break;
            case "k": moment.moveMonth(-1);
                break;
            case "l": moment.moveMonth(1);
                break;
            case "m": moment.moveYear(-1);
                break;
            case ",": moment.moveYear(1);
                break;
            case ".": moment.moveYear(-50);
                break;
            case "/": moment.moveYear(50);
                break;
            default:
